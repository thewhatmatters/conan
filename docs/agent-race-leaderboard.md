...
|-------|-------|--------|-----------------|-----------------------------|---------|-------|
| p2c | US-301..US-305 Full composer | Claude | `thewhatmatters/v2-p2c-claude` @ `0b22a72` | `loop/conan-v2-astryx` @ `d60c3fa` / tag `checkpoint/v2-p2c` | http://127.0.0.1:5208 | Pins drawer, branch chip, provider·model·effort picker, @// input + paste/drop; 81/81 vitest; typecheck/build clean; v1 untouched. Post-checkpoint fix: model picker trigger now re-enables `pointer-events: auto` so it stays clickable when ChatComposer is disabled. Minor UI refinements planned before final. |
