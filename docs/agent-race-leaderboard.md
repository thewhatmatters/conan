...
|-------|-------|--------|-----------------|-----------------------------|---------|-------|
| p2c | US-301..US-305 Full composer | Claude | `thewhatmatters/v2-p2c-claude` @ `0b22a72` | `loop/conan-v2-astryx` @ `731f8b0` / tag `checkpoint/v2-p2c` | http://127.0.0.1:5208 | Pins drawer, branch chip, provider·model·effort picker, @// input + paste/drop; 91/91 vitest; typecheck/build clean; v1 untouched. Post-checkpoint: model picker trigger pointer-events fix, then Claude one-off rebuilt picker to 122-1 + real SVG brand marks + effort chip split + fixed-height panel. |
