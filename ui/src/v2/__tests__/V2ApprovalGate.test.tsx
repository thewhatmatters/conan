...
describe("V2ApprovalGate", () => {
  ...
  // Geometry (action row stays inside the viewport when options overflow) cannot
  // be asserted in jsdom: getBoundingClientRect() returns zeros there. The
  // structural test above guarantees the action row is not nested inside the
  // scrollable body; the actual layout check must happen in a real browser.
});
